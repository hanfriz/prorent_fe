...
export default function RoomTypeAccordion({ roomTypes, startDate, endDate, onReservationPageChange }: RoomTypeAccordionProps) {
      ...
      {roomTypes.map((roomType) => {
        ...
        return (
          <div key={roomType.roomType.id} className="border rounded-lg">
            ...
            <div className="p-4">
              <div className="flex items-center justify-between">
                <span className="font-medium text-lg">{roomType.roomType.name}</span>
                <div className="flex gap-4 text-sm">                 
                  <span>  Pending Bookings:{" "}  {(    (roomType.counts?.PENDING_PAYMENT || 0) +    (roomType.counts?.PENDING_CONFIRMATION || 0)  )}</span>
                </div>
              </div>
              <div className="mt-3">
            ...
            <Accordion type="single" collapsible className="w-full border-t">
              <AccordionItem value={`${roomType.roomType.id}-calendar`} className="border-0">
                <AccordionTrigger className="hover:no-underline px-4 py-3 [&[data-state=open]>div:last-child>svg]:rotate-45">
                  <div className="flex items-center justify-between w-full">
                    ...
                    <span> {roomType.roomType.name} - {roomType.availability.totalQuantity} total units</span>
                  </div>
                </AccordionTrigger>
          ...
          <Accordion type="single" collapsible className="w-full border-t">
            <div className="border-1 rounded-2xl mb-4"></div>
                  <AccordionItem value={`${roomType.roomType.id}-reservations`} className="border-0">
        ...
        <AccordionContent className="p-4 pt-0">
          <RoomTypeReservationsTable
            ...
            onPageChange={(page) => onReservationPageChange(roomType.roomType.id, page)}
          />
        </AccordionContent>
      </AccordionItem>
           </Accordion>
