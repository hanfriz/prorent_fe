...
import { prepareChartData, getChartOptions } from './chartComponent/chartConfig';
...
export default function Graph({ 
  ...
  // Initialize hooks
  const { data, rawData, isLoading, error, refetch, invalidateAll } = useChartData();
  const { 
    chartSettings, 
    setViewMode, 
    setChartType, 
  ...
  useEffect(() => {
    if (chartSettings.chartType !== 'line') {
      console.log('🔄 [Graph Component] Setting default chart type to line');
      setChartType('line');
    }
  ...
  const handleRefresh = async () => {
    console.log('🔄 [Graph Component] Manual refresh triggered');
    setChartError(null);
    await invalidateAll();
    const result = await refetch();
    console.log('✅ [Graph Component] Refresh completed:', result);
  };

  useEffect(() => {
    console.log('📈 [Graph Component] Chart data updated:', {
  ...
  if (chartError || error) {
    console.log('❌ [Graph Component] Showing error state', { chartError, error });
    return <ChartError error={chartError || error} onRetry={handleRefresh} />;
  }
  
  // No data state - show for the entire component
  if (!isLoading && (!data || data.length === 0)) {
    console.log('📭 [Graph Component] Showing no data state');
    return (
        ...
        {showControls && (
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6 gap-4">
            <div>
              ...
              <p className="text-gray-600">Track your actual vs projected revenue performance</p>
            </div>
  ...
  const chartJsData = data ? prepareChartData(data, chartSettings.chartType) : null;
  const options = getChartOptions(chartSettings.chartType);
  
  console.log('🖼️ [Graph Component] Rendering chart with type:', chartSettings.chartType);
